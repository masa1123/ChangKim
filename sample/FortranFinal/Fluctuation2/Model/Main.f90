...
program Fluctuations
	...
	real(8) errK, errK2, errR, errW, err
	...
	do iterLOM = 1, maxiterLOM
		...
		write(0, '(/A, <ncoef>F10.5)') "Law of Motion for Kapital:", Kcoef
		write(0, '(/A, <ncoef>F10.5)') "Law of Motion for K2     :", K2coef
		write(0, '(/A, <ncoef>F10.5)') "Interest Rate Equation   :", Rcoef
		write(0, '(/A, <ncoef>F10.5)') "Wage Rate Equation       :", Wcoef
		write(0, '(//A,I2,A,A,A)') "SolveValueFunction in Iteration ",	iterLOM, " started at ", today, solvetime
		...
		call date(today)
		simultime = clock()
		write(0, '(/A,I2,A,A,A)') "SimulateData in Iteration ",	 iterLOM, " started at ", today, simultime
		...
		errK2 = maxval(dabs(NewK2coef - K2coef))
		errR  = maxval(dabs(NewRcoef - Rcoef))
		errW  = maxval(dabs(NewWcoef - Wcoef))
		err   = max(errK, errK2, errR, errW)

		Kcoef = sfac*NewKcoef + (1.0D0 - sfac)*Kcoef
		K2coef = sfac*NewK2coef + (1.0D0 - sfac)*K2coef
...
!		if (err < tol_LOM) then
			...
			write(0, '(/A,F10.6,A,F10.6,A,F10.6)')	"errK = ", errK, "    errK2 = ", errK2, "    errR = ", errR, "    errW = ", errW
			...
			write(0, '(//A,I2)') "Errors in LOM Iteration ", iterLOM
			write(0, '(/A,F10.6,A,F10.6,A,F10.6,A,F10.6)')	"errK = ", errK, "    errK2 = ", errK2, "    errR = ", errR, "    errW = ", errW
		end if
